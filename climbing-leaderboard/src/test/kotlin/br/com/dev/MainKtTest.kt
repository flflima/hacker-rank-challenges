...
internal class MainKtTest {
    ...
    @Test
    fun `test with a small leader board`() {
        val result = Main.climbingLeaderboard(
            listOf(100, 90, 90, 80, 75, 60).toTypedArray(),
            listOf(50, 65, 77, 90, 102).toTypedArray()
        )

        assertThat(result).isEqualTo(listOf(6, 5, 4, 2, 1).toTypedArray())
    }

    @Test
    fun `test with a medium leader board`() {
        val result = Main.climbingLeaderboard(
            listOf(100, 90, 90, 80, 75, 60).toTypedArray(),
            listOf(50, 65, 77, 90, 102, 50, 88).toTypedArray()
        )

        assertThat(result).isEqualTo(listOf(6, 5, 4, 2, 1, 6, 3).toTypedArray())
    }

    @Test
    fun `test with a large leader board`() {
        val result = Main.climbingLeaderboard(
